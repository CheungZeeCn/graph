...
def save_dataloader(path="new_dataset/trainer.tsv", mode="e_s_c_i", df_frac=1,
    ...
    if create_validation > 0:
        train_inputs, validation_inputs, train_labels, validation_labels, train_masks, validation_masks = train_test_split(
            input_ids, labels, attention_masks, test_size=create_validation, shuffle=False)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', metavar='M', type=str, default="e_s_ci",
                        help='classifier mode: e_s_c_i (or) e_s_ci (or) e_sci (or) es_ci')
    parser.add_argument('--lm', metavar='LM', type=str, default="xlm", help='language model: xlm (or) bert')
    parser.add_argument('--maxlen', metavar='ML', type=int, default=128,
                        help='max length of the sentence to be considered')
    parser.add_argument('--split_mark', metavar='SM', type=str, default="train",
                        help='marks the type of split; train, test or valid')
    parser.add_argument('--create_validation', metavar='CV', type=float, default=0.05,
                        help='fraction of the train data to be used for validation')
